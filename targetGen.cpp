...
bool tokenSearch(Node* node, std::string input) {
	for(int i=0; i < ((int)node->Tokens.size() - (int)1) ; ++i) {
		if (node->Tokens[i].instance == input)
			return true;
	}
	return false;
}
...
void printToFile(const char* code, ...) {
	 ...
	 * vprintf -> http://www.cplusplus.com/reference/cstdio/vprintf/
	 */
	va_list args;
	va_start(args, code);
...
bool Traverse(Node* node) {
	...
	if(Label ("<Program>") ) {
		...
		if(node->Leaves.empty()) {
			...
			// 	vars.front() = node->Tokens.back();
			// 	vars.back().instance = node->Tokens[1].instance;

			// 	std::string text = node->Tokens[3].instance;;
			// 	int number = std::atoi (text.c_str());

			// 	vars.back().line = number;
			// 	printToFile("LOAD V%d", (int)vars.size() );
			// 	if(!Traverse(node->Leaves[0]) ) {
			// 		return false;
			// 	}
			// 	return true;
	 ...
	 */
		...
		if( node->Leaves.size() == 2 ) {
			...
			if(node->Tokens[0].instance == "-") {
				...
				return true;
				// tempToken.id = Token::Id::IntegerTk;
				// tempToken.instance = "0";
				// tempToken.line = -1;
				// vars.push_back(tempToken);
			...
			if(node->Tokens[0].instance == "+") {
				...
				printf("ERROR: expr has multiple args without + or -.\n");
				return false;
			}
		} else {
			if(!Traverse(node->Leaves[0]) ) {
		...
		if(node->Tokens.size() == 1 ) {
			...
			// 	printToFile("LOAD T%i", (int)temps.size() );
			// }

			// if(!node->Leaves[0]->Tokens.empty() ) {
			// 	if(!node->Leaves[0]->Tokens[0].instance.empty() ) {
			// 		printToFile("STORE V%i", (int)vars.size() );
			// 	}
			// }
		...
		if( node->Leaves.empty() ) {
			...
			if(node->Tokens[0].id == Token::integerTk) {
				//printf("R: integer case\n");
				printToFile("LOAD %s", node->Tokens[0].instance.c_str());
				return true;
			} else {
				//printf("R: var case\n");
				//printToFile("STORE V%i", (int)vars.size() );
				int rHelper = 0;
				rHelper = findVar(node->Tokens[0].instance.c_str());
		...
		// vars.back().line = 0;
		int place = findVar(node->Tokens[1].instance.c_str());
		//printf("Place: %i\n", place);
		...
		// if(node->Tokens[1].id == Token::identifierTk) {
		// 	printToFile("STORE V%i", (int)vars.size() );
		// 	printToFile("WRITE V%i", (int)vars.size() );
		// 	return true;
		// }
			// if(vars.empty()) {
			// 	temps.push_back(0);
		...
		if(node->Leaves[1]->Tokens.size() > 1) {
			...
			// if(node->Leaves[1]->Tokens[0].instance == "<"){
			// 	if(node->Leaves[1]->Tokens[1].instance == "="){
			// 		printToFile("BRNEG label%i", label1);
			// 	} else if (node->Leaves[1]->Tokens[1].instance == ">") {
			// 		printToFile("BRZERO label%i", label1);
			// 	}
			// } else if (node->Leaves[1]->Tokens[0].instance == ">") {
			// 	if(node->Leaves[1]->Tokens[1].instance == "="){
		...
		if(node->Leaves[1]->Tokens.size() == 2) {
			...
			// if(node->Leaves[0]->Tokens[0].instance == "=") {
		...
		if(!Traverse(node->Leaves[0])){
			...
			int assignHelper = findVar(node->Tokens[0].instance.c_str());
			if(assignHelper != -1) {
